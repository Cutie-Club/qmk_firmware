...
enum custom_keycodes {
  ADOBE_KEYCODE = USER00,
};

#define ADOBE_DELAY_MS 1000

bool adobe_pause_timer_running = false;
uint16_t adobe_pause_timer = 0;
...
void matrix_scan_user(void) {
  if(is_adobe_pause_active) {
    if (timer_elapsed(alt_tab_timer) > ADOBE_DELAY_MS){
      ...
      adobe_pause_timer_running = false;
    }
  }
}
  ...
  switch (keycode) {
    case ADOBE_KEYCODE:
      if (record->event.pressed) {
        ...
        tap_code(KC_M);
        tap_code(KC_ESC);
        adobe_pause_timer_running = true;
        adobe_pause_timer = timer_read();

        // blocking alternative
        // SEND_STRING(SS_TAP(X_M) SS_TAP(X_M) SS_TAP(X_ESC) SS_DELAY(1000) SS_LALT(SS_TAP(X_1)));
      }
      break;
  }
  return true;
